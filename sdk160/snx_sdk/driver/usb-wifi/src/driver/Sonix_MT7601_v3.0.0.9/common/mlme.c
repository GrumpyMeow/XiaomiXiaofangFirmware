	...
	IN PVOID SystemSpecific3) 
{
	...
	if(test_str_suspend_enable(pAd) == TRUE){
		DBGPRINT(RT_DEBUG_ERROR,("MT7601U has suspend, should not do anything,because some time USB host have sleep.\n"));
		return;
	}
 ...
 && (pAd->Multi_Channel_Enable == FALSE)
	...
	*/
	...
	if (P2P_INF_ON(pAd))
	{
				...
				/* suspect when using GO as softAP, do not need P2pPeriodicExec */
				if(!GO_SOFTAP_ON(pAd))
	...
	IN UCHAR	 Channel) 
{
	...
	for (i = 0; i < Tab->BssNr; i++) 
	{
		...
		/* We should distinguish this case.*/
		/*		*/
		/* Note: If SSID or BSSID match either of one, we consider OK */
		if ((((Tab->BssEntry[i].Channel <= 14) && (Channel <= 14)) ||
			 ((Tab->BssEntry[i].Channel > 14) && (Channel > 14))) &&
			(MAC_ADDR_EQUAL(Tab->BssEntry[i].Bssid, pBssid) ||
			SSID_EQUAL(pSsid, SsidLen, Tab->BssEntry[i].Ssid, Tab->BssEntry[i].SsidLen))) 
		{ 
			RTMPMoveMemory(pBssid, Tab->BssEntry[i].Bssid, MAC_ADDR_LEN);
			return i;
		}
	}
	return (ULONG)BSS_NOT_FOUND;
