...
function initial_language() {
    ...
    $("#add-ip-page-title").text(LVA_Add_Page_Title);
    $("#general_text").text(LVA_Add_Page_Title);
    $("#ip_addr_text").text(LVA_Add_Page_IP);
    $("#ok_btn").val(LVA_Add_Page_btn_OK);
    $("#cancel_btn").val(LVA_Add_Page_btn_Cancel);
}
    ...
    $("form").submit(function () {
        ...
        $.each(ip_addresses, function (index, elem) {
            if (elem == $("#ip_addr").val().replace(/(^\s*)|(\s*$)/g, "")) {
                alert(elem + LVA_Add_Page_existIP_warning);
                $("#ip_addr").focus().select();
                ret = false;
                return;
            }
