...
#ifdef WSC_AP_SUPPORT
#ifdef RT_CFG80211_SUPPORT
/* in cfg80211 mode, wpa_supplicant may provide WPS IE */
    if (pAd->ApCfg.MBSSID[pEntry->apidx].AssocRespExtraIeLen &&
        RTMPFindWPSIE(pAd->ApCfg.MBSSID[pEntry->apidx].AssocRespExtraIe, 
                      pAd->ApCfg.MBSSID[pEntry->apidx].AssocRespExtraIeLen))
    {
        /* wpa_supplicant has provided the WPS IE in ExtraIe,
           so driver does not need to add WPS IE.*/
    }
    else    
	...
	if (ie_list->P2PSubelementLen > 0)
	{
		...
		P2pParseSubElmt(pAd, (PVOID)ie_list->P2pSubelement, ie_list->P2PSubelementLen, FALSE, &Dpid, &GroupCap, 
			...
			&ConfigMethod, DeviceType, NULL, NULL, NULL, NULL, (UCHAR *)&StatusCode, NULL, NULL);
