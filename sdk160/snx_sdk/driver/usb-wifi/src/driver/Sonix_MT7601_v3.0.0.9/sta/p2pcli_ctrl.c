...
//use for normal concurrent case!
					...
					//if (pAd->Multi_Channel_Enable == TRUE)
					{
...

							{
								...
								//move up, set bw to 20 before switch channel
...
//use for normal concurrent case!
			...
			//if (pAd->Multi_Channel_Enable == TRUE)
			{
					...
					/* Patch for P2P cert 5.1.13.
					   The Testbed P2P GO has distinct MAC addresses for P2P Device Addr and
					   P2P Interface Addr. In such case, because current driver persistent
					   table entry only has P2P Device Addr stored. It will cause we do not
					   try to associate to GO after Invitation was completed.
					*/
					/* In P2P persistent case, we compare either BSSID or SSID, because DevAddr and IfAddr may be different */
					//bss_idx = BssSsidTableSearch(&pAd->ScanTab, JoinReq.Bssid, (PCHAR)(JoinReq.Ssid), JoinReq.SsidLen, pAd->CommonCfg.Channel);
					bss_idx = BssTableSearch_SsidOrBssid(&pAd->ScanTab, JoinReq.Bssid, (PCHAR)(JoinReq.Ssid), JoinReq.SsidLen, pAd->CommonCfg.Channel);
					DBGPRINT(RT_DEBUG_TRACE, ("(%s) Bssid=[%02x:%02x:%02x:%02x:%02x:%02x]\n", __FUNCTION__,
						JoinReq.Bssid[0], JoinReq.Bssid[1], JoinReq.Bssid[2], 
						JoinReq.Bssid[3], JoinReq.Bssid[4], JoinReq.Bssid[5]));
					if (!MAC_ADDR_EQUAL(pApCliEntry->CfgApCliBssid, JoinReq.Bssid))
					{
						COPY_MAC_ADDR(pApCliEntry->CfgApCliBssid, JoinReq.Bssid);
					}
					/* End of Patch for P2P cert 5.1.13. */
