	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (pAd->StaCfg.AuthMode >= Ndis802_11AuthModeWPAPSK)
	{
		...
		printk("\x1b[31m%s: aid %d\x1b[m\n", __FUNCTION__, pAd->StaActive.Aid);
		DBGPRINT(RT_DEBUG_TRACE,("	Key = %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n",
	...
	printk("\x1b[31m%s: disable usb interrupt \x1b[m\n", __FUNCTION__);
	RtmpOsMsDelay(1);
	...
	printk("\x1b[31m%s: Value %x\x1b[m\n", __FUNCTION__, Value);
	
	if (Value < 160)
		RtmpOsMsDelay(20); /* delay for across beacon boundary */

	RTMP_IO_READ32(pAd, TBTT_TIMER, &Value);
	printk("\x1b[32m%s: Value %x\x1b[m\n", __FUNCTION__, Value);

	NextDTIM = pAd->StaCfg.DtimCount - 1;
	NextDTIM = NextDTIM >= 0 ? NextDTIM : pAd->StaCfg.DtimPeriod - 1;

	AndesPwrSavingOP(pAd, RADIO_OFF_ADVANCE, 0x1, pAd->CommonCfg.BeaconPeriod, pAd->StaCfg.DtimPeriod, NextDTIM, 0);

	printk("\x1b[33m%s: DTIM period %d, beacon period %d, next DTIM %d, TBTT Timer %x\x1b[m\n", 
					__FUNCTION__, 
...
/*
Patch start
	1. Disable UDMA TX/RX                  // Set 0x41_0238 = 0
*/
	DBGPRINT(RT_DEBUG_ERROR, ("1. Disable UDMA TX/RX\n"));

	RTMP_IO_FORCE_READ32(pAd, 0x0238, &MACValue);
	DBGPRINT(RT_DEBUG_ERROR, ("0x41_0238:%08x\n",MACValue));

//save mac 0238 for Disable WOW.
	MAC0238_WOW_DATA = 0x238;

	MACValue = 0;
	RTMP_IO_FORCE_WRITE32(pAd, 0x0238, MACValue);
	RTMP_IO_FORCE_READ32(pAd, 0x0238, &MACValue);
	DBGPRINT(RT_DEBUG_ERROR, ("0x41_0238:%08x\n",MACValue));
/*
Patch end
...
#ifdef NEW_WOW_REBOOT_USB_POWERON_SUPPORT
    if(0 != MAC0238_WOW_DATA){
    	RTMP_IO_FORCE_WRITE32(pAd, 0x0238, MAC0238_WOW_DATA);
    	MAC0238_WOW_DATA=0;
    }
#endif
