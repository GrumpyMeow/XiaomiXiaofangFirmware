...
BOOLEAN Set_ATE_Load_Buffer_From_Efuse_Proc(
	IN	PRTMP_ADAPTER	pAd, 
	IN	PSTRING			arg)
{
	UINT32 value = simple_strtol(arg, 0, 10);
	...
	if (value == 1)
	{
		if (LoadBufferFromEfuse(pAd) == TRUE)
		{
...
BOOLEAN Set_ATE_Load_Buffer_From_BIN_Proc(
	IN	PRTMP_ADAPTER	pAd, 
	IN	PSTRING			arg)
{
	BOOLEAN	    	ret = FALSE;
	...
	if (value > 0)
	{
		...
		do
		{
			...
			/* read the firmware from the file *.bin */
			FileLength = RtmpOSFileRead(srcf, (PSTRING)pAd->EEPROMImage, MAX_EEPROM_BIN_FILE_SIZE);

			if (FileLength != MAX_EEPROM_BIN_FILE_SIZE)
			{
				DBGPRINT_ERR(("%s : error file length (=%d) in .bin\n",
					   __FUNCTION__, FileLength));
				break;
			}
			else
	...
	IN UCHAR 			Channel)
{
	...
	if ((pATEInfo->bIQCompEnable == TRUE) && ((E2PValue & 0x00FF) != 0x00FF))
	{
		ATE_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R159, E2PValue & 0x00FF);
	}
	else
	{
		ATE_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R159, 0x00);
	}
