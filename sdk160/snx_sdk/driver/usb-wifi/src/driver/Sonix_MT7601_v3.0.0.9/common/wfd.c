	...
	switch (SubId)
	{
		case SUBID_WFD_DEVICE_INFO:
		{
			...
			if (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_MEDIA_STATE_CONNECTED) && (INFRA_ON(pAd)))
				#ifdef CONFIG_MSTARTV_CUS_SUPPORT	
				DevInfo.PC_ = pWFDCtrl->PC_;
				#else
				DevInfo.PC = pWFDCtrl->PC;
				#endif
			else
				#ifdef CONFIG_MSTARTV_CUS_SUPPORT
				DevInfo.PC_ = WFD_PC_P2P;
				#else
		...
		{
			if (pWFDCtrl->DeviceType != WFD_SOURCE )
			{
				...
				tmpValue = cpu2be16(EidLen);
				RTMPMoveMemory(pDest, &tmpValue, 2);
				if (pWFDCtrl->SinkCoupled == WFD_COUPLED_SUPPORT)
				{
				...
				*/
				...
				for (i = 1; i < MAX_LEN_OF_MAC_TABLE; i++)
				{
					...
					if ((P2pIdx < MAX_P2P_GROUP_SIZE) && (Tab->Client[P2pIdx].WfdEntryInfo.bWfdClient == TRUE))
					{
						...
						#ifdef CONFIG_MSTARTV_CUS_SUPPORT	
						SessionInfo.WfdDevInfo.PC_ = pAd->P2pTable.Client[P2pIdx].WfdEntryInfo.wfd_PC;
						#else
