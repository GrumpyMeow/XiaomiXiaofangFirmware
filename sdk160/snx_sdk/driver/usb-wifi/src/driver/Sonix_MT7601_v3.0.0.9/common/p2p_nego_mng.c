		...
		IN MLME_QUEUE_ELEM *Elem)
{
	...
	else if (pP2PCtrl->Rule == P2P_IS_CLIENT)
	{ //for 7.1.1
		/* Invite Case 2 : I am client to invite a P2P Device */
		P2pInvite(pAd, pP2pCmd->Addr, MAX_P2P_TABLE_SIZE, p2pIndex);
	}
	else if (IS_P2P_CONNECT_IDLE(pAd))
	{
		...
		if ((perstindex < MAX_P2P_TABLE_SIZE) && (IS_PERSISTENT_ON(pAd)))
		{
			...
			/* Patch for P2P cert: 5.1.13. 
			   Invitation Rsp may be received too early as expected, so need to raise this flag earlier.
			 */
