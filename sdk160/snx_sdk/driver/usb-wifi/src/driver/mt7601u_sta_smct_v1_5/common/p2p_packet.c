	...
	OUT VOID *pPrivate)
{
	...
	while ((ULONG)(AccuIeLen) <= MsgLen)
	{
		if (RTMPEqualMemory(&pEid->Octet[0], WPS_OUI, 4))
		{
			...
			if (bBeacon == TRUE)
				P2PParseWPSIE((PUCHAR)(&pEid->Octet), (USHORT)(pEid->Len + 2), pDpid, pWpsConfigMethod, pDeviceName, pDeviceNameLen);
			else
				P2PParseWPSIE((PUCHAR)(&pEid->Octet),  (USHORT)(pEid->Len + 2), pDpid, pWpsConfigMethod, NULL, NULL);
		}
		/* might contains P2P IE and WPS IE.  So use if else if enough for locate  P2P IE. */
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					pAddr = (PUCHAR)(&pAd->P2pTable.Client[p2pindex].addr);
	...
	IN PUCHAR		Addr1)
{
		...
		if (pP2pEntry->Rule == P2P_IS_CLIENT)
		{
			...
			pP2PCtrl->SSIDLen = pAd->ApCfg.MBSSID[MAIN_MBSSID].SsidLen;
			{
				...
				TempLen = InsertP2PSubelmtTlv(pAd, SUBID_P2P_GROUP_ID, (PUCHAR)(&tmpGroupID), pDest);
			}
			FrameLen += TempLen;
			pDest += TempLen;
			P2pLen +=TempLen;
	...
	*pTotalFrameLen += Length;
	...
	if (index != P2P_NOT_FOUND)
	{
		...
		if (pP2pEntry->Rule == P2P_IS_GO) // peer is GO
		{
			...
			Length = InsertP2PSubelmtTlv(pAd, SUBID_P2P_GROUP_ID, (PUCHAR)(&tmpGroupID), pDest);
