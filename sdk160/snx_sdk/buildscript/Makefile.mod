...
fftw3: 
	...
	$(MAKE) -C $(PRJ_DIR)/app/fftw3 all COMPILE_RESCUE=no
	$(MAKE) -C $(PRJ_DIR)/app/fftw3 install INSTALL_DIR=$(PRJ_DIR)/app/fftw3/rootfs COMPILE_RESCUE=no
	if [ -d $(PRJ_DIR)/app/fftw3/rootfs ]; then \
		cp -av $(PRJ_DIR)/app/fftw3/rootfs/* $(FS_DIR); \
	fi
ifeq ($(MODULE_ONLY), yes)
fftw3-clean:
else
fftw3-clean: 
	...
	$(MAKE) -C $(PRJ_DIR)/app/fftw3 clean
	$(MAKE) -C $(PRJ_DIR)/app/fftw3 distclean
	rm -rf $(PRJ_DIR)/app/fftw3/rootfs
ifeq ($(CONFIG_RESCUE_SYSTEM), y)
	rm -rf $(PRJ_DIR)/app/fftw3/rootfs-rescue
endif
