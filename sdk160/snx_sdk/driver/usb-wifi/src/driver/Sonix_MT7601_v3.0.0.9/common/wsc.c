...
{		
	...
	else if (MsgType == WSC_MSG_EAP_FAIL)
	{
				...
				{					
					if (sendFail)
					{
						...
						WidiUpdateStateToDaemon(pAdapter, MIN_NET_DEVICE_FOR_MBSSID, WIDI_MSG_TYPE_ASSOC_STATUS, 
												...
												0, 0, 0
#endif /* WIDI_GEN4_SUPPORT */
												, NULL, 0, WIDI_WPS_STATUS_SUCCESS);					
				}
#endif /* WIDI_SUPPORT */				
			}
#endif /* CONFIG_STA_SUPPORT */
		...
		else if (pWscControl->WscState == WSC_STATE_RX_M2D)
		{
			...
			{
				pWscControl = &pAdapter->ApCfg.ApCliTab[apidx].WscControl;
				WscApCliLinkDown(pAdapter, pWscControl);
			}
#endif /* APCLI_SUPPORT */
			...
			bNonceMatch)		
		{
			...
			if (MsgType == WSC_MSG_M2)
			{
				...
				if (P2P_CLI_ON(pAdapter) && (pWscControl->EntryIfIdx != BSS0))
				{
					...
					if (P2pIdx != P2P_NOT_FOUND)
					{
						pP2pEntry = &pAdapter->P2pTable.Client[P2pIdx];
				...
				(pWscControl->WscConfStatus == WSC_SCSTATE_CONFIGURED))
			{
				...
				if (IS_PERSISTENT_ON(pAdapter) && (pEntry->bP2pClient == TRUE))
				{
					...
					if ((P2pIdx != P2P_NOT_FOUND) && (IS_P2P_GO_ENTRY(pEntry) || IS_P2P_CLI_ENTRY(pEntry)))
					{
						...
						P2pPerstTabInsert(pAdapter, pP2pEntry->DeviceAddr, &pWscControl->WscProfile.Profile[0]);
...
Done:
	...
	if (rv == 1)
	{
			...
			)
		{
            ...
            WidiUpdateStateToDaemon(pAdapter, MIN_NET_DEVICE_FOR_MBSSID, WIDI_MSG_TYPE_ASSOC_STATUS, pEntry->Addr,
#ifdef WIDI_GEN4_SUPPORT
			0, 0, 0
#endif /* WIDI_GEN4_SUPPORT */
			, NULL, 0, WIDI_P2P_WPS_SUCCESS);
#endif /* WIDI_SUPPORT */
				...
				*/
				...
				if (IS_PERSISTENT_ON(pAdapter) && !isZeroEtherAddr(PersistentEntry.Addr) && (PersistentEntry.bP2pClient == TRUE))
				{
					...
					if ((P2pTabClientIdx != P2P_NOT_FOUND) && (IS_P2P_GO_ENTRY(pEntry) || IS_P2P_CLI_ENTRY(pEntry)))
					{
						...
						P2pPerstTabInsert(pAdapter, P2pClientEntry.DeviceAddr, &pWscControl->WscProfile.Profile[0]);
						// this is a persistent connection.
						pAdapter->P2pCfg.ExtListenInterval = P2P_EXT_LISTEN_INTERVAL;
						pAdapter->P2pCfg.ExtListenPeriod = P2P_EXT_LISTEN_PERIOD;
					}
...
#ifdef P2P_SUPPORT
					/* Shorten P2P connection time */
					if (P2P_GO_ON(pAdapter) && IS_P2P_GO_ENTRY(pEntry))
						RTMPSetTimer(&pWscControl->WscUpdatePortCfgTimer, 0);
					else
#endif /* P2P_SUPPORT */
					RTMPSetTimer(&pWscControl->WscUpdatePortCfgTimer, 1000);
					pWscControl->WscUpdatePortCfgTimerRunning = TRUE;
