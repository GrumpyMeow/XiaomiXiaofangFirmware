  ...
  IN RTMP_ADAPTER   *pAd)
{
  ...
  INT16 RSSI_THR0 = pAd->CommonCfg.RssiThr0; //Default: -25dBm
  ...
  if (pAd->ApCfg.MBSSID[MAIN_MBSSID].StaCount < 2)
  {
    /*get default rssi_thr*/  
      if (rssi > RSSI_THR0)//~-25  R65/R66:  0x25/0x34
      {
        init_main_vga += 0x20;  //R66: 0x34 (0x14 will fail sometimes)
        init_aux_vga += 0x20;
	init_lna_R65 = 0x25; /*BBP_R65[3:2] from 3 into 2 : 0x25*/
      }
      else if (rssi > RSSI_THR1)//-25~-45  R65/R66:  0x2c/0x34
      {
	init_main_vga += 0x20;  //R66: 0x34
        init_aux_vga += 0x20;
	init_lna_R65 = 0x2c; /*BBP_R65[3:2] from 3 into 2 : 0x2c*/
      }
  ...
  DBGPRINT(RT_DEBUG_TRACE, ("[%s]LNA R65(0x%x),main_vga(0x%x),aux_vga(0x%x),rssi(%d),StaCount(%d)\n",
          __FUNCTION__, init_lna_R65, init_main_vga, init_aux_vga, rssi, pAd->ApCfg.MBSSID[MAIN_MBSSID].StaCount));
