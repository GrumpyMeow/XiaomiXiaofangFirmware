...
ULONG InsertP2PDefaulConfigMethods(
	IN PRTMP_ADAPTER pAd,
	IN PUCHAR		pInBuffer,
	IN PUCHAR		pOutBuffer)
{
		...
		*(pDest + 1) = 0;
		...
		RTMPMoveMemory(pDest + 2, pInBuffer, MAC_ADDR_LEN);
		/* DEvice Type */
		*((PUSHORT) (pDest + 8)) = cpu2be16(WSC_CONFMET_DEFAULT);
		RTMPMoveMemory(pDest + 10, &pP2PCtrl->DevInfo.PriDeviceType[0], 8);
	...
	OUT PUCHAR 	pDest)
{
...
#endif /* P2P_CHANNEL_LIST_SEPARATE */
	{
        ...
        if(INFRA_ON(pAd)) // insert ra0's infra channel
        {
                CurRegClass = ChannelToClass(pAd->CommonCfg.Channel, 1);
                LastRegClass = CurRegClass;
                *(pDest) = CurRegClass;


                /* Insert  */
                *(pDest + pos) = pAd->CommonCfg.Channel;
                LastNum++;
                pos++;
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	if (index < MAX_P2P_GROUP_SIZE)
	{
		...
		for (k = 0;k < MAX_NUM_OF_CHANNELS;k++)
		{
			...
			if (ChannelList[k] > 14 )
			{
		#ifdef CONFIG_MSTARTV_CUS_SUPPORT //for mstar tcl6308: select 5G channel as opchannel.
		...
		*/
		WidiUpdateStateToDaemon(pAd, MIN_NET_DEVICE_FOR_P2P_CLI, WIDI_MSG_TYPE_ASSOC_STATUS, pP2PCtrl->Bssid,
#ifdef WIDI_GEN4_SUPPORT
		0, 0, 0,
#endif /* WIDI_GEN4_SUPPORT */
		pP2pEntry->Ssid, pP2pEntry->SsidLen, WIDI_P2P_GO_CONFIRM_RECEIVED);
		...
		if (pAd->P2pCfg.bWIDI)
		{
			UINT32 DataLen = 0;
			PUCHAR pData = NULL;
			os_alloc_mem(NULL, (UCHAR **)&pData, 256);
			if (pData)
			{
				P2pGetWfdIE(pAd, &pFrame->ElementID, (Elem->MsgLen + 7 - sizeof(P2P_PUBLIC_FRAME)), pData, &DataLen);
				if (DataLen != 0)
					WidiNotifyWfdIEToDaemon(pAd, pData, DataLen, pFrame->p80211Header.Addr2, WFD_IE_FROM_GO_NEGO_CONFIRM);
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	UCHAR		GroupCap, DeviceCap, DeviceName[P2P_DEVICE_NAME_LEN + 1];
...
#ifdef WIDI_GEN4_SUPPORT
	/* Disable re-transmit on WIDI Gen4 */
		#ifndef CONFIG_MSTARTV_CUS_SUPPORT //for mstar tcl6308: need to resend .
	if (pP2PCtrl->bWIDI!=TRUE) //add for mstar tcl6308
		#endif
#endif /* WIDI_GEN4_SUPPORT */
	{
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
		...
		UCHAR		GroupCap, DeviceCap, DeviceName[P2P_DEVICE_NAME_LEN + 1], DeviceNameLen;
		ULONG		FrameLen;
		ULONG		TempLen;
		UCHAR		SsidLen = 0;
		...
		*/
		...
		#endif
		RTMPZeroMemory(&DeviceName[0], sizeof(DeviceName));
		pP2pEntry = NULL;
...
#ifndef WIDI_GEN4_SUPPORT
/* WIDI Gen4 : allways reply by user */
#ifdef WIDI_SUPPORT
				|| (pAd->P2pCfg.bWIDI == TRUE)
#endif /* WIDI_SUPPORT */
#endif /* WIDI_GEN4_SUPPORT */
				)
			{
			...
			pP2pEntry->DeviceName[DeviceNameLen] = '\0';
			pP2pEntry->ConfigMethod = ConfigMethod;
			pP2pEntry->bSupport5G = FALSE;
			pP2pEntry->bSCCChannel=FALSE;
			if (!MAC_ADDR_EQUAL(DevAddr, ZERO_MAC_ADDR))
			...
			if (WfdSubelementLen)
			{
				...
				*/
				...
				pP2pEntry->WfdEntryInfo.session_avail=session_avail_backup;
                     ...
                     {
                             /* reset WfdEntryInfo to zero so that App won't be confused between WFD and Pure-P2P */
                             if (pP2pEntry)
                                        RTMPZeroMemory(&pP2pEntry->WfdEntryInfo, sizeof(WFD_ENTRY_INFO));
                     }
...
#ifdef WIDI_SUPPORT
/* Gen4 pass all status code include P2PSTATUS_SUCCESS / P2PSTATUS_PASSED */
#ifndef WIDI_GEN4_SUPPORT
		if (RspStatus == P2PSTATUS_SUCCESS)
#endif /* WIDI_GEN4_SUPPORT */
		{
			WidiUpdateStateToDaemon(pAd, MIN_NET_DEVICE_FOR_P2P_CLI, WIDI_MSG_TYPE_ASSOC_STATUS, DevAddr,
#ifdef WIDI_GEN4_SUPPORT
		0, Dpid, RspStatus,
#endif /* WIDI_GEN4_SUPPORT */
				NULL, 0, WIDI_P2P_GO_REQ_RECIVED);
			...
			if (pAd->P2pCfg.bWIDI)
			{
				...
				if (pData)
		{
					P2pGetWfdIE(pAd, &pFrame->ElementID, (Elem->MsgLen - LENGTH_802_11 - 8), pData, &DataLen);
					if (DataLen != 0)
						WidiNotifyWfdIEToDaemon(pAd, pData, DataLen, pFrame->p80211Header.Addr2, WFD_IE_FROM_GO_NEGO_REQ);
					os_free_mem(NULL, pData);
				}
			}
#endif /* WFA_WFD_SUPPORT */
...
#ifdef WIDI_GEN4_SUPPORT
		 //Gen4, passed should not call P2pSetRule()!, just let user space daemon trigger the connect.
		if (pP2PCtrl->bWIDI != TRUE)
#endif /* WIDI_GEN4_SUPPORT */
		{
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
...
#ifdef WIDI_SUPPORT
		 else if ( RspStatus == P2PSTATUS_PASSED ) {
		      bReinvoke = FALSE;
...
#ifdef CONFIG_MSTARTV_CUS_SUPPORT
	/*	theoretically,we should switch to ListenChannel of peer,to make sure
		peer can receive our invite rsp. but we can also delay to wait peer  on this channel
	 */	
	#if 0
	pAd->CommonCfg.Channel = pP2pEntry->ListenChannel;
    ...
    /* speed up the invitation rsp, send event after packet is processed */
#ifdef RT_P2P_SPECIFIC_WIRELESS_EVENT
	...
	{
		#ifdef WIDI_GEN4_SUPPORT
		 /* Gen4, Let daemon trigger provision, instad of driver! */
		if (pP2PCtrl->bWIDI !=TRUE)
		#endif /* WIDI_GEN4_SUPPORT */
	{
		pP2pEntry->P2pClientState = P2PSTATE_DISCOVERY_GO;
	...
	else if ((bReinvoke == TRUE) && (RspStatus == P2PSTATUS_SUCCESS))
	{
		...
		if (MyRule == P2P_IS_CLIENT)
		{
			...
			/* DON'T copy the P2P device address to BSSID because peer Bssid may be different with the peer P2P device address */
...
#ifdef P2P_GC_OP_CH_FIRST
			pP2PCtrl->bP2pOPChannelConnect = TRUE;
#endif
		...
		else
		{
			if (!P2P_GO_ON(pAd) && !P2P_CLI_ON(pAd))
			{
				...
				/*
					For LG-N7-Pad and samsung s4 they will switch to it`s listenChannel after sending invite 
					request fame. so we should send rsp on that channel,but the stayTime will be hard to control
					so,we can wait for it come to current channel again and receive our rsp successfully.
					by sniffer, found that 500 ms is appropriate.
				 */
			#ifdef CONFIG_MSTARTV_CUS_SUPPORT
				OS_WAIT(500);
			#else
...
#ifdef RT_P2P_SPECIFIC_WIRELESS_EVENT
				/* When invitation has invoked and we are as GO, send P2P-GROUP-STARTED event to upper layer */
...
//Gen4  : modify Invite recv event
#ifdef WIDI_SUPPORT
		/*
			For mstar widi sdk,after receiving P2PSTATUS_UNKNOWN_GROUP status code,it will start provision
			request and force driver to switch to channel-11,driver wiil send negotiation rsp on that channel
			but the phone can`t receive. so negotiation will fail.
  		 */
		#ifdef CONFIG_MSTARTV_CUS_SUPPORT
			 if( RspStatus == P2PSTATUS_UNKNOWN_GROUP || RspStatus == P2PSTATUS_PASSED)
			{
				char ComputerWin7Type[6]={0x00,0x01,0x00,0x50,0xf2,0x04};
		//for mstar tcl6308: win8.1 pc widi link fail
        char ComputerWin8Type[6]={0x00,0x01,0x00,0x50,0xf2,0x00};
				if( DevType )
			 {
					printk("dev info Category:%02x %02x,OUI:%02x %02x %02x %02x",
							DevType[0],DevType[1],DevType[2],DevType[3],DevType[4],DevType[5]);
					if( RTMPCompareMemory(ComputerWin7Type,DevType,6)==0 )
					{
						printk("peer device is intel widi,it should send provision req first\n");
					}
          else if( RTMPCompareMemory(ComputerWin8Type,DevType,6)==0 )
          {
            printk("peer device is win8.1 intel widi,it should send provision req first\n");
          }
					else
				 RspStatus = P2PSTATUS_SUCCESS;
			 }
             ...
             if ( RspStatus == P2PSTATUS_SUCCESS || RspStatus == P2PSTATUS_UNKNOWN_GROUP || RspStatus == P2PSTATUS_PASSED )
             {
		WidiUpdateStateToDaemon(pAd, MIN_NET_DEVICE_FOR_P2P_CLI, WIDI_MSG_TYPE_ASSOC_STATUS, 
								pFrame->p80211Header.Addr2,
#ifdef WIDI_GEN4_SUPPORT
		0, 0, RspStatus,
#endif /* WIDI_GEN4_SUPPORT */
								Ssid, SsidLen, WIDI_P2P_INVITE_REQ_RECEIVED);
             }
#endif /* WIDI_SUPPORT */
	...
	if ((StatusCode == P2PSTATUS_SUCCESS) || (StatusCode == P2PSTATUS_PASSED))
	{
		...
		else
		{
			...
			if (index < MAX_P2P_GROUP_SIZE)
			{
				...
				{
					//for 7.1.1 reset for P2pGroupMaintain()
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	UCHAR		DeviceName[P2P_DEVICE_NAME_LEN + 1], DeviceNameLen;
	UCHAR		DevType[8];
...
#endif
	pP2PCtrl->bConfirmByUI = pP2PCtrl->bP2pAutoAccept;
	RTMPZeroMemory(&DeviceName, sizeof(DeviceName));
	P2pParseSubElmt(pAd, &pFrame->ElementID, (Elem->MsgLen - LENGTH_802_11 - 8), 
...
if (P2P_GO_ON(pAd))
	...
	*/
	...
	if ( (WpsConfigMethod == WSC_CONFMET_DISPLAY)
		...
		|| (WpsConfigMethod == WSC_CONFMET_DEFAULT))
	{
		...
		if (p2pindex < MAX_P2P_GROUP_SIZE)
		{
			...
			if ( pP2PCtrl->bProvAutoRsp == TRUE)
			{
				...
				WidiUpdateStateToDaemon(pAd, MIN_NET_DEVICE_FOR_P2P_CLI, WIDI_MSG_TYPE_ASSOC_STATUS,
						...
						WpsConfigMethod, 0, 0,
#endif /* WIDI_GEN4_SUPPORT */
						NULL, 0, WIDI_P2P_PROVISION_REQ_RECV);
			...
			if (pP2PCtrl->bWIDI)
			{
				...
				if (pData)
				{
					P2pGetWfdIE(pAd, &pFrame->ElementID, (Elem->MsgLen - LENGTH_802_11 - 8), pData, &DataLen);
					if (DataLen != 0)
						WidiNotifyWfdIEToDaemon(pAd, pData, DataLen, pFrame->p80211Header.Addr2, WFD_IE_FROM_PROVISION_REQ);
					os_free_mem(NULL, pData);
				}
			}
#endif /* WFA_WFD_SUPPORT */
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
...
#ifdef WIDI_GEN4_SUPPORT
	/* Disable re-transmit on WIDI Gen4 */
		#ifndef CONFIG_MSTARTV_CUS_SUPPORT //for mstar tcl6308: select 5G channel as opchannel.
	if (pP2PCtrl->bWIDI!=TRUE) //add for mstar tcl6308
		#endif
#endif /* WIDI_GEN4_SUPPORT */
	{
	...
	/* point to OUI2 */
#if 0 /* WSC IE may be not the only one and the first IE */
	...
	P2pParseSubElmt(pAd, &pFrame->ElementID, (Elem->MsgLen - LENGTH_802_11 - 8), 
		...
		NULL, NULL, NULL, NULL, &ConfigMethod, NULL, 
		...
		|| (ConfigMethod == WSC_CONFMET_PBC))
	{
		...
		else
		{
			...
			else
			{
				if ((pP2PCtrl->ConfigMethod == WSC_CONFMET_KEYPAD) || 
#ifdef WIDI_GEN4_SUPPORT
					((pP2PCtrl->bWIDI == TRUE) && (pP2PCtrl->ConfigMethod == WSC_CONFMET_PBC)) ||
#endif /* WIDI_GEN4_SUPPORT */
				...
				{
#ifdef WIDI_GEN4_SUPPORT
					 //Gen4 : DUT should not trigger go nego by driver, just triggered by daemon.
					if (pP2PCtrl->bWIDI != TRUE)
#endif /* WIDI_GEN4_SUPPORT */
					{
					/* After Provision Success, start connect command */
...
*/	
		...
		if (pAd->P2pCfg.bWIDI)
		{
			if (pAd->pWfdIeInActionPkt && (pAd->WfdIeInActionPktLen != 0))
			{
				RTMPMoveMemory(pDest, pAd->pWfdIeInActionPkt, pAd->WfdIeInActionPktLen);
				TempLen = pAd->WfdIeInActionPktLen;
				pDest += TempLen;
				FrameLen += TempLen;
			}
		}
	...
	OUT PULONG		pTotalFrameLen)
{
	...
	*pTotalFrameLen += Length;
	...
	if (pAd->P2pCfg.bWIDI)
	{
		...
		{
			RTMPMoveMemory(pDest, pAd->pWfdIeInActionPkt, pAd->WfdIeInActionPktLen);
			Length = pAd->WfdIeInActionPktLen;
			pDest += Length;
			*pTotalFrameLen += Length;
		}
	}
	...
	OUT PULONG		pTotalFrameLen)
{
	...
	if(InviteFlag)
	{
		if(MyRule == P2P_IS_CLIENT)
		{
			RTMPMoveMemory(&tmp_MAC,Addr1,MAC_ADDR_LEN);
		}
		else if(MyRule == P2P_IS_GO)
		{
			RTMPMoveMemory(&tmp_MAC,&pP2PCtrl->CurrentAddress[0],MAC_ADDR_LEN);
		}
		else
		{
			DBGPRINT(RT_DEBUG_ERROR, ("P2PMakeInviteReq revoke on : ERROR!! %02x:%02x:%02x:%02x:%02x:%02x. \n", PRINT_MAC(Addr1)));
		}
	}
	else
	{
		...
		else if (P2P_CLI_ON(pAd))
		{
			...
			DBGPRINT(RT_DEBUG_ERROR, ("P2PMakeInviteReq normal invite [chia-chi] CLI_ON : BSSID %02x:%02x:%02x:%02x:%02x:%02x. \n", PRINT_MAC(pP2PCtrl->Bssid)));
		
			RTMPMoveMemory(&tmp_MAC,&pP2PCtrl->Bssid[0],MAC_ADDR_LEN);
		}
		else
		{
			DBGPRINT(RT_DEBUG_ERROR, ("P2PMakeInviteReq normal invite: ERROR!! %02x:%02x:%02x:%02x:%02x:%02x. \n", PRINT_MAC(Addr1)));			
		}
	}
	...
	*pTotalFrameLen += Length;
	...
	else if ((MyRule == P2P_IS_CLIENT) && P2P_CLI_ON(pAd))
	{
	...
	/* attach subelementID= 7. */
	Length = InsertP2PSubelmtTlv(pAd, SUBID_P2P_GROUP_BSSID, &tmp_MAC[0], pDest);
	pDest += Length;
	P2pIeLen += Length;
	*pTotalFrameLen += Length;
	/* attach subelementID= 11. */
	...
	*pTotalFrameLen += Length;
	/* attach subelementID= 11. */
	{
		...
		tmpGroupID[1] = 0;
		RTMPMoveMemory(&tmpGroupID[2], &tmp_MAC[0], MAC_ADDR_LEN);	
...
#ifdef WFD_SUPPORT
#ifdef WIDI_SUPPORT
				if (pAd->P2pCfg.bWIDI && NdisEqualMemory(pEid->Octet, WFA_WFD_OUI, 4) && (pEid->Len >= 4))
				{
					hex_dump("WFD_IE", &pEid->Eid, pEid->Len +2);
					RTMPMoveMemory(pWfdSubelement, &pEid->Eid, pEid->Len +2);
					*pWfdSubelementLen = pEid->Len + 2;					
					WidiNotifyWfdIEToDaemon(pAd, (pEid->Octet), (pEid->Len), pAddr2, WFD_IE_FROM_PROBE_REQ);
				}
#endif /* WIDI_SUPPORT */
#endif /* WFD_SUPPORT */
				break;
...
/*	IN UCHAR	WorkingChannel) */
{
		...
		UCHAR		DeviceNameLen = 0, DeviceName[P2P_DEVICE_NAME_LEN + 1];
			...
			if (index < MAX_P2P_GROUP_SIZE)
			{
				...
				if (DeviceNameLen != 0)
				{
					...
					if (bNewFound)
					{
						...
						*/
						RTMPMoveMemory(&pAd->P2pTable.Client[index].DeviceName[0], DeviceName, DeviceNameLen);
						pAd->P2pTable.Client[index].DeviceNameLen = DeviceNameLen;
						pAd->P2pTable.Client[index].DeviceName[DeviceNameLen] = '\0';
					}
...
#else /* DPA_S */
					RTMPMoveMemory(&pP2pEntry->DeviceName[0], DeviceName, DeviceNameLen);
					pP2pEntry->DeviceNameLen = DeviceNameLen;
					pP2pEntry->DeviceName[DeviceNameLen] = '\0';
#endif /* !DPA_S */
				}
...
//QQ TBD, p2p widi toggle scan need reply response!
#ifdef WIDI_SUPPORT
		if (pP2PCtrl->bWIDI == FALSE)
#endif /* WIDI_SUPPORT */
		{
									...
									WpsIE))
		{
			...
			if (index < MAX_P2P_GROUP_SIZE)
			{
				P2pParseSubElmt(pAd, P2pSubelement, P2PSubelementLen, 
					FALSE, NULL, &pAd->P2pTable.Client[index].GroupCapability, &pAd->P2pTable.Client[index].DevCapability, DeviceName, &DeviceNameLen, DevAddr,
				...
				if (DeviceNameLen != 0)
				{
					RTMPZeroMemory(&pAd->P2pTable.Client[index].DeviceName[0], sizeof(pAd->P2pTable.Client[index].DeviceName));
					RTMPMoveMemory(&pAd->P2pTable.Client[index].DeviceName[0], DeviceName, DeviceNameLen);
					pAd->P2pTable.Client[index].DeviceNameLen = DeviceNameLen;
				}
	...
	OUT	PULONG			pIeLen)
{
	...
	*((PUSHORT) (pData + 4)) = cpu2be16(WSC_CONFMET_DEFAULT);
	...
	OUT	PUCHAR			pIeLen)
{
	...
	*((PUSHORT) pData) = cpu2be16(WSC_ID_CONFIG_METHODS);
	*((PUSHORT) (pData + 2)) = cpu2be16(0x0002);
	*((PUSHORT) (pData + 4)) = cpu2be16(WSC_CONFMET_DEFAULT);/* Keypad, Display, PBC */
	pData += 6;
	Len   += 6;
	...
	*((PUSHORT) (pData + 2)) = cpu2be16(pP2PCtrl->DeviceNameLen);
	...
	/* 13. DPID shall be a required attribute if Credentials are available and ready for immediate use. */
	{
		USHORT Dpid = WSC_CONFMET_DEFAULT;
		/* Device Password ID */
		*((PUSHORT) pData) = cpu2be16(WSC_ID_DEVICE_PWD_ID);
		*((PUSHORT) (pData + 2)) = cpu2be16(0x0002);
...
#endif
	if (DsChannel == 0)
		#ifdef CONFIG_MSTARTV_CUS_SUPPORT //for mstar tcl6308: Dschannel=0.
	{
		DBGPRINT(RT_DEBUG_ERROR, ("SYNC - P2PMakeProbeRsp. DsChannel = is 0 !!!!!\n"));
		DsChannel = pAd->CommonCfg.Channel;
		DBGPRINT(RT_DEBUG_ERROR, ("SYNC - P2PMakeProbeRsp. DsChannel = %d\n",DsChannel));
...
#if 0 /* remove HT Capability in P2P find phase to fix S4 can not find issue */
	...
	OUT	PULONG			pIeLen)
{
	...
	*pIeLen = 0;
	...
	if ((PacketType == SUBTYPE_PROBE_RSP) || (PacketType == SUBTYPE_ASSOC_REQ) || (PacketType == SUBTYPE_REASSOC_REQ))
	{
		...
		/* Must show the default comfig methods in P2P device info subelement IE to let peer P2P device know our capability */
		TempLen = InsertP2PDefaulConfigMethods(pAd, pP2PCtrl->CurrentAddress, pData);
		Len += TempLen;
		pData += TempLen;
	}
...
#if 0 /* Remove BSSID checking, the BSSID may be different with the device address for some P2P devices */
	...
	OUT UINT32			*pWfdIeLen) 
{
	...
	while ((ULONG)(AccuIeLen) <= MsgLen)
	{
		if (RTMPEqualMemory(&pEid->Octet[0], WFA_WFD_OUI, 4))
		{
			if (pWfdIeLen)
				*pWfdIeLen = pEid->Len;
			
			if (pWfdIe)
				NdisMoveMemory(pWfdIe, &pEid->Octet[0], pEid->Len);

			DBGPRINT(RT_DEBUG_ERROR, (" P2pGetWfdIE - Found WFA_WFD_OUI, return.\n"));
			return;
		}
	...
	for (iPeerChannelIndex = 0;iPeerChannelIndex < MAX_NUM_OF_CHANNELS;iPeerChannelIndex++)
	{
		...
		( !(INFRA_ON(pAd) || P2P_GO_ON(pAd) || P2P_CLI_ON(pAd))))
		{
			for (i=0;i<MyChannelListNum;i++)
			{
				...
				{		
					#if 0//def CONFIG_MSTARTV_CUS_SUPPORT//
					if ( (pMyChannelList[i].Channel == 1)||
						 (pMyChannelList[i].Channel == 6)||
						  (pMyChannelList[i].Channel == 11))
					{
						pChannelInfo->dirtyness[i] += 50;
					}
					
					DBGPRINT(RT_DEBUG_OFF, ("%s[%d] check channel[%d]= %d,dirtyness=%d \n", __FUNCTION__,__LINE__,
						iPeerChannelIndex, ChannelList[iPeerChannelIndex],
						pChannelInfo->dirtyness[i]));
					#endif

					// find a best channel from matching channels
					if ((min_dirty > pChannelInfo->dirtyness[i]))
					{
						#ifdef CONFIG_MSTARTV_CUS_SUPPORT// filter out 12 13 14 ,because many phone can not support these channels
						if(//(pMyChannelList[i].Channel !=12) &&
							//(pMyChannelList[i].Channel !=13) &&
							(pMyChannelList[i].Channel !=14)
							)
						#endif
						{
						...
						DBGPRINT(RT_DEBUG_TRACE, ("%s[%d] min_dirty = %d, PeerChannelListNum = %d\n", \
									__FUNCTION__,__LINE__, pChannelInfo->dirtyness[i], pMyChannelList[i].Channel));
						}
					}
					#ifdef CONFIG_MSTARTV_CUS_SUPPORT
					break;
