	...
	IN PRTMP_ADAPTER pAd) 
{
	...
	/* WIDI WIN8 NB won't trigger Go Nego to DUT, if DUT's probe resp is set to configured */
	pP2PCtrl->bConfiguredAP = FALSE;
#else /* WIDI_GEN4_SUPPORT */
...
#ifdef WIDI_GEN4_SUPPORT
	/* Gen4 need to trigger by daemon, not driver auto */
	pP2PCtrl->bP2pAutoAccept = FALSE;
#else /* WIDI_GEN4_SUPPORT */
...
#ifdef WIDI_SUPPORT	
/* removed, and need widi daemon use "iwpriv p2p0 set p2pPerEnable=1"  to turn on */
#if 0//def INTEL_WFD_SUPPORT
	pP2PCtrl->P2pControl.word = 0x8030800C;
#endif /* INTEL_WFD_SUPPORT */
	pP2PCtrl->bWIDI = FALSE;
	...
	pP2PCtrl->GoConfigTimeout = P2P_DEFAULT_GO_CONFIG_TIMEOUT;
	pP2PCtrl->GcConfigTimeout = P2P_DEFAULT_GC_CONFIG_TIMEOUT;
...
#ifdef HE_SUPPORT
	/* Fix when user sets "p2p_lisent" to enter Listen State, driver does not actullay stay on ListenChannel */
	else
	{
		pAd->MlmeAux.Channel = 0;
		ScanNextChannel(pAd, OPMODE_STA, StopChannel);
	...
	OUT PUCHAR pSSIDLen)
{
	...
	if (pAd->P2pCfg.DeviceNameLen <= (MAX_LEN_OF_SSID - 9))
	{
	...
	{
		NdisMoveMemory((pSSID + 9), pAd->P2pCfg.DeviceName, (MAX_LEN_OF_SSID - 9));
		(*pSSIDLen) = MAX_LEN_OF_SSID;
	}
}
	...
	IN UINT32 ConnType)
{
    ...
    #ifndef CONFIG_MSTARTV_CUS_SUPPORT //for mstar tcl6308: peer opchannel and listen channel =0.
    ...
    {       
        Channel = pAd->P2pTable.Client[p2pindex].OpChannel;      
        DBGPRINT(RT_DEBUG_TRACE, ("P2pConnectPrepare():send channel(GC)=%d!\n",Channel));      
        if(Channel==0)      
        {        
            Channel = pAd->P2pTable.Client[p2pindex].ListenChannel;      
        }     
    ...
    {       
        Channel = pAd->P2pTable.Client[p2pindex].ListenChannel;      
        DBGPRINT(RT_DEBUG_TRACE, ("P2pConnectPrepare():send channel(GO)=%d!\n",Channel));       
        if(Channel==0)      
        {        
            Channel = pAd->P2pTable.Client[p2pindex].OpChannel;      
        }    
    }     
    DBGPRINT(RT_DEBUG_TRACE, ("P2pConnectPrepare():send channel=%d!\n",Channel));     

    if(Channel==0)      
    {       
        Channel = pAd->P2pCfg.ListenChannel;        
        DBGPRINT(RT_DEBUG_ERROR, ("P2pConnectPrepare():send channel=%d!\n",Channel));      
    }
    #endif
	...
	IN UCHAR		idx)
{
	...
	if (idx < MAX_P2P_GROUP_SIZE)
	{
		...
		#ifndef CONFIG_MSTARTV_CUS_SUPPORT //for mstar tcl6308: nego request send fail,need to resend.
            ...
            if (pP2PCtrl->Rule == P2P_IS_GO)
            {
                        if( pP2PCtrl->AssocList.maclist_count < 3)
                        {
                                    ...
                                    for ( j = 0 ; j < pP2PCtrl->AssocList.maclist_count ; j ++ )
                                                if ( NdisEqualMemory(&pP2PCtrl->AssocList.device_name[j].ssid , pP2pEntry->DeviceName, P2P_DEVICE_NAME_LEN) )
                                                            is_found = TRUE;
				...
				NdisMoveMemory(&pP2PCtrl->AssocList.device_type[i].dev_type, &pP2pEntry->PrimaryDevType, 8);
                                               NdisMoveMemory(&pP2PCtrl->AssocList.maclist[i].octet, &pP2pEntry->InterfaceAddr, MAC_ADDR_LEN);
                                                NdisZeroMemory(&pP2PCtrl->AssocList.device_name[i].ssid, P2P_DEVICE_NAME_LEN + 1);
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	{
		pP2PCtrl->ConfigMethod = WSC_CONFMET_DEFAULT;
		pAd->StaCfg.WscControl.RegData.SelfInfo.ConfigMethods = WSC_CONFMET_DEFAULT;
		pAd->StaCfg.WscControl.RegData.SelfInfo.DevPwdId = WSC_CONFMET_DEFAULT;
	}
	...
	UINT32	value)
{
	...
	if (pP2PCtrl->P2pCounter.bStartScan == FALSE)
	{
		...
		if (pAd->ToggleEnabled && pP2PCtrl->bWIDI)
		{
...
/*	BOOLEAN		bDot1xReload = FALSE; */
 ...
 /*When go softAP on, disalbe p2pGO auto set security*/
 if (!GO_SOFTAP_ON(pAd))
 {
...
*/
	...
	/* GO configuration timeout of P2P group owner should be 0*/
	pAd->P2pCfg.ConfigTimeout[0] = 0;
	...
	IN PRTMP_ADAPTER pAd) 
{
	...
	/* Reset GO configuration timeout while not being P2P group owner */
	pAd->P2pCfg.ConfigTimeout[0] = pAd->P2pCfg.GoConfigTimeout;
	...
	IN PRTMP_ADAPTER 	pAd)
{
	...
	/* Client configuration timeout of P2P group client should be 0*/
	pAd->P2pCfg.ConfigTimeout[1] = 0;
	return;
}
	...
	IN PRTMP_ADAPTER 	pAd)
{
	...
	/* Reset Client configuration timeout while not being P2P group client */
	pAd->P2pCfg.ConfigTimeout[1] = pAd->P2pCfg.GcConfigTimeout;
}
