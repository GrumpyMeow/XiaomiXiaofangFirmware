	...
	uint8 *mac_addr, uint32 flags, uint8 bssidx)
{
	...
	if (ifp != NULL) {
		...
		if ((ifp = MALLOC(dhd->pub.osh, sizeof(dhd_if_t))) == NULL) {
			DHD_ERROR(("%s: OOM - dhd_if_t(%d)\n", __FUNCTION__, (int)sizeof(dhd_if_t)));
			return -ENOMEM;
		}
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	if (!disable_proptx)
		...
		dhd->pub.wlfc_enabled = FALSE;
#endif /* PROP_TXSTATUS_VSDB */
#else /* defined(BCMSDIO) || defined(BCMDBUS) */
	dhd->pub.wlfc_enabled = FALSE;
#endif 
#ifdef BCMDBUS
	dhd->pub.ptx_opt_enabled = TRUE;
#else
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	uint32 nmode = 0;
#endif /* DISABLE_11N */
#if defined(VSDB) && defined(OEM_ANDROID) && defined(CUSTOMER_HW4)
...
#else
#if defined(BCMDBUS) && defined(PROP_TXSTATUS) && defined(PROP_TXSTATUS_VSDB)
...
#endif /* defined(OEM_ANDROID) && defined(BCMLXSDMMC) && (LINUX_VERSION_CODE >= */
