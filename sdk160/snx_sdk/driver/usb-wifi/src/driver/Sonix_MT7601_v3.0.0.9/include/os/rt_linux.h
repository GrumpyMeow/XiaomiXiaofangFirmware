...
/*
	this branch derived from CE_MT7601U with changeList CL97063.version information:
	MT7601_0422_CL97063_ACH_DY_OCP_BASAS_LGS4IOT_HPDPN means that these features had been enabled as follows:
	ACH:	p2p auto channel select expect from 12 13 14 
	DY:		Dynamic vga used to enhance anti-interference performance
	OCP:	pmu ocp patch
	BASAS:	ba win size auto switch between station and wfd with value 64 & configuration from dat
	LGS4IOT:For customer changhong,LG-N7-PAD and samsung can`t connect sink with miracast£¬
			case1:After receiving invite request,widi sdk switch to channel-11,so peer can not receive 
			invite rsp from 7601. fixed within this version
			case2:After receiving invite req with mispatching ssid from peer,driver will send 
			P2PSTATUS_UNKNOWN_GROUP or P2PSTATUS_PASSED status code to widi daemon,at the same time
			sending status code to peer within invite rsp .Then peer will send negotiation req,but widi 
			sdk had switched to other channel to start provision request so that negotiation will fail.
			fixed within this version
	HPDPN:	If the process of remod triggered by user has not been finished, when usr plug the dongle,system
			will crash.Add protection mechanism within MCUCtrlExit\rt28xx_init.Fixed now
 */
#define MS_WIFI_VERSION			"MT7601_0422_CL97063_ACH_DY_OCP_BASAS_LGS4IOT_HPDPN"
#define STA_PROFILE_PATH			"/etc/Wireless/RT2870STA/RT2870STA_7601.dat"
#else
#define STA_PROFILE_PATH			"/etc/Wireless/RT2870STA/RT2870STA.dat"
#endif
#endif /*SEPARATE_PROFILE_SUPPORT */
#ifdef CONFIG_MSTARTV_CUS_SUPPORT
#define STA_DRIVER_VERSION			"MT7601_20141202_CL114182"
#else
#define STA_DRIVER_VERSION			"MT7601_v3.0.0.9_140416_96234"
#endif
#ifdef MULTIPLE_CARD_SUPPORT
