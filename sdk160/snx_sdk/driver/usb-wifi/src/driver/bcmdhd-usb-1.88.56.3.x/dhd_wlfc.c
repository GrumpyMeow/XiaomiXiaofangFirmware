...
static uint32
_dhd_wlfc_tx_pkts_available(athost_wl_status_info_t* ctx);
...
_dhd_wlfc_tx_pkts_available(athost_wl_status_info_t* ctx)
{
	...
	for (i = 0; i < total_entries; i++) {
		entry = &table[i];
		if (entry->occupied && pktq_len(&entry->psq)) {
			return TRUE;
		}
	}
	return FALSE;
...
#ifdef BCMDBUS
	dhd_pub_t* dhdp = (dhd_pub_t*) commit_ctx;
#endif
				...
				if (rc == BCME_OK) {
					...
					if (bus_retry_count >= BUS_RETRIES) {
						DHD_ERROR((" %s: bus error %d\n",
							__FUNCTION__, rc));
#ifdef BCMDBUS
						if (dhdp->ptx_opt_enabled)
							dhdp->pending_tx_pkts =
								_dhd_wlfc_tx_pkts_available(ctx);
#endif
						return rc;
					}
				}
...
#ifdef BCMDBUS
			if (dhdp->ptx_opt_enabled)
				dhdp->pending_tx_pkts = _dhd_wlfc_tx_pkts_available(ctx);
#endif
			return BCME_OK;
		}
	}
