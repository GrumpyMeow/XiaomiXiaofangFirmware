...
{	
	...
	#ifdef USB_BULK_BUF_ALIGMENT //      CONFIG_MSTARTV_SUPPORT
			 int bufferSize =0;
			 bufferSize += ((sizeof(HTTX_BUFFER)*NUM_OF_TX_RING * BUF_ALIGMENT_RINGSIZE));
			 bufferSize += (sizeof(TX_BUFFER)*2);
			 bufferSize += (MAX_RXBULK_SIZE * RX_RING_SIZE);
			 bufferSize += CMD_RSP_BULK_SIZE;
			 
			 DBGPRINT(RT_DEBUG_OFF,("------->driver will alloc dma buffer information as follows \n"
																	   "HTTX_BUFFER 			0x%x Bytes=%d * %d\n"
																		"TX_BUFFER				 0x%x Bytes=%d * %d\n"
																		"RXBULK 				 0x%x Bytes=%d * %d\n"
																		"CMD_RSP				 0x%x Bytes=%d * %d\n"
																		"all buffer size		 %dK  Bytes\n"
																		"------------end\n"
																			   ,(sizeof(HTTX_BUFFER)*NUM_OF_TX_RING*BUF_ALIGMENT_RINGSIZE)
																			   ,sizeof(HTTX_BUFFER), NUM_OF_TX_RING*BUF_ALIGMENT_RINGSIZE
																			   ,(sizeof(TX_BUFFER)*2),sizeof(TX_BUFFER),2
																			   ,(MAX_RXBULK_SIZE * RX_RING_SIZE),MAX_RXBULK_SIZE,RX_RING_SIZE
																			   ,CMD_RSP_BULK_SIZE,CMD_RSP_BULK_SIZE,1
																			   ,( bufferSize/1024 )
																			   ));
    #endif
