...

{
	...
	{
		LONG baSize = RTMPGetBasizeFromConfdat();
		pAd->CommonCfg.REGBACapability.field.RxBAWinLimit = baSize;
		pAd->CommonCfg.BACapability.field.RxBAWinLimit = baSize;
		DBGPRINT(RT_DEBUG_OFF,("#####wfd mode we switch ba win size to %d config within *.dat\n",baSize));
	}
	#endif
