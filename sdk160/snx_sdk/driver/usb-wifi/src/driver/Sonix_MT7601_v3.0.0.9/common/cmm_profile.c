...
static LONG baWinSizeCfgFromDat = 0;
#endif
...
*/
#ifdef CONFIG_MSTARTV_CUS_SUPPORT
LONG RTMPGetBasizeFromConfdat()
{
	return baWinSizeCfgFromDat;
}
#endif
	...
	IN	PSTRING		  pInput)
{
    ...
    if (RTMPGetKeyParameter("HT_BAWinSize", pValueStr, 25, pInput, TRUE))
	{
		...
		#ifdef CONFIG_MSTARTV_CUS_SUPPORT
		baWinSizeCfgFromDat = pAd->CommonCfg.REGBACapability.field.RxBAWinLimit;
		#endif
	}
...
#ifdef DFS_SUPPORT
	{
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
		{
			...
			if(RTMPGetKeyParameter("P2pAutoChannelSelect", tmpbuf, 10, pBuffer, TRUE))
			{
				...
				{
					ChannelSel_Alg SelAlg=(ChannelSel_Alg)simple_strtol(tmpbuf, 0, 10);
					if ((SelAlg & P2P_AUTO_CHANNEL_ENABLE) == 0)
					{
					...
					else /*Enable*/
					{
						...
						pAd->P2pCfg.bAutoChOnSocial = (SelAlg & P2P_AUTO_CHANNEL_SOCIAL_CHS_ONLY) ? TRUE : FALSE;

						// check if auto ch on 5G first
						pAd->P2pCfg.bAutoChOn5G = (SelAlg & P2P_AUTO_CHANNEL_5G_SUPPORT) ? TRUE : FALSE;
			...
			if(RTMPGetKeyParameter("GoConfigTimeout", tmpbuf, 10, pBuffer, TRUE))
			{
				pAd->P2pCfg.GoConfigTimeout= simple_strtol(tmpbuf, 0, 10);
				DBGPRINT(RT_DEBUG_ERROR, ("GoConfigTimeout=%d\n", pAd->P2pCfg.GoConfigTimeout));
			}
			if(RTMPGetKeyParameter("GcConfigTimeout", tmpbuf, 10, pBuffer, TRUE))
			{
				pAd->P2pCfg.GcConfigTimeout= simple_strtol(tmpbuf, 0, 10);
				DBGPRINT(RT_DEBUG_ERROR, ("GcConfigTimeout=%d\n", pAd->P2pCfg.GcConfigTimeout));
			}
