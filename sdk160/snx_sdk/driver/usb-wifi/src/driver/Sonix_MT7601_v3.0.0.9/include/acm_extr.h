...
#define ACMR_AP_TXOP_BSS_GET(__pAd, __AcId)	\
	...
	/* check if WMM is enabled */
#define ACMR_AP_WMM_IS_ENABLED(__pAd)	(__pAd)->ApCfg.MBSSID[BSS0].bWmmCapable
...
#define ACMR_STA_PS_MODE_RECOVER(__pAd)										\
	{																		\
		...
		{																	\
			...
			if (!((__pAd)->StaCfg.UapsdInfo.bAPSDCapable &&					\
...
#define ACMR_APSD_CAPABLE_GET(__pAd)				\
		__pAd->StaCfg.UapsdInfo.bAPSDCapable
...
#define ACMR_LIST_ENTRY				LIST_ENTRY
