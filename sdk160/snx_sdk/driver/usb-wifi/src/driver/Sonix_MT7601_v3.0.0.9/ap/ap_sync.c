		 ...
		 */
{
	...
	/* don't need to carry p2p IE when we are softap */
	if (!GO_SOFTAP_ON(pAd))
...
//p2p client need to send ps when p2p client want scan!
			...
			&&(pAd->CommonCfg.Channel == pAd->LatchRfRegs.Channel) )
		{
			...
			DBGPRINT(RT_DEBUG_TRACE, ("APMlmeScanReqAction -- Send PSM Data frame for off channel RM, SCAN_IN_PROGRESS=%d!\n",
