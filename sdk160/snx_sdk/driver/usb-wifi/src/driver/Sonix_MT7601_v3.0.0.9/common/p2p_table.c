	...
	IN PRTMP_ADAPTER pAd)
{
		...
		&& (pAd->P2pCfg.P2pCounter.Wpa2pskCounter100ms > 1200/*GOSCANBEGINCOUNTER_MAX*/))
	{
		...
		||(pP2PCtrl->P2PConnectState == P2P_CONNECT_IDLE))
#endif
	else if (((pAd->flg_p2p_OpStatusFlags == 0) || (P2P_CLI_ON(pAd)))&&
	...
	IN UCHAR		Macindex)
{
	...
	RTMPMoveMemory(pP2pEntry->DeviceName, pEntry->P2pInfo.DeviceName, pEntry->P2pInfo.DeviceNameLen);
	pP2pEntry->DeviceName[pP2pEntry->DeviceNameLen] = '\0';
	...
	IN UCHAR		Macindex)
{
	...
	RTMPMoveMemory(pEntry->P2pInfo.DeviceName, pP2pEntry->DeviceName, sizeof(pP2pEntry->DeviceName));
	pEntry->P2pInfo.DeviceName[pP2pEntry->DeviceNameLen] = '\0';
	...
	IN PRTMP_ADAPTER pAd) 
{
	...
	{		
		...
		RTMPZeroMemory(Tab->Client[i].DeviceName, sizeof(Tab->Client[i].DeviceName));
	...
	{		
		...
		if (pP2pEntry->P2pClientState >= P2PSTATE_CLIENT_OPERATING)
		{
			...
			*/
			...
			RTMPZeroMemory(pP2pEntry->DeviceName, sizeof(pP2pEntry->DeviceName));
	...
	IN PUCHAR    Addr) 
{
	...
	if (index < MAX_P2P_GROUP_SIZE)
	{
		...
		/* Fix driver sent "P2P-PROV-DISC-PBC-REQ" event with wrong MAC (00:00:00:55:00:00) to app.
		   We should not set byte[3] as 0x55 for nothing. */
		/*
                     ...
                     if (index < MAX_P2P_TABLE_SIZE)
                     {
                                           ...
                                           (NewDeviceNameLen != 0 ) && (NewDeviceNameLen <= P2P_DEVICE_NAME_LEN))
                                {
                                          // check if there is update on device name
                                           ...
                                           {
                                                     //clear device name
	                                             RTMPZeroMemory(Tab->PerstEntry[i].DeviceName, sizeof(pAd->P2pTable.PerstEntry[0].DeviceName));
                                                     //update device name
...
write_entry:
			...
			/* If persistent table is full, we overwrite to a random idx entry */
			else if (i == (MAX_P2P_TABLE_SIZE-1))
			{
				i = (RandomByte(pAd) % MAX_P2P_TABLE_SIZE);
				Tab->PerstEntry[i].bValid = FALSE;
				DBGPRINT(RT_DEBUG_ERROR, ("Persistent Table is full, overwriting idx[%d]\n", i));
				goto write_entry;
			}
		}
	}
	...
	IN UCHAR	*pMacList) 
{
	...
	{		
		if (Tab->PerstEntry[i].bValid == TRUE)
		{
			if (NdisEqualMemory(Tab->PerstEntry[i].Addr, pMacList, 6))
			{
				...
				{
					Tab->PerstNumber--;
					DBGPRINT(RT_DEBUG_ERROR, (" P2P - Delete a Persistent Entry[%d] .Table Number = %d. \n",i ,Tab->PerstNumber));
					return i;
				}
				else
	...
	DBGPRINT(RT_DEBUG_ERROR, (" P2P - Persistent table MAC not found. \n"));
