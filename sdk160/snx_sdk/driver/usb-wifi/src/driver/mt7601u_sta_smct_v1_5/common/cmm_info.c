	...
	IN	PSTRING			arg)
{
	...
	Value = (UINT16)simple_strtol(arg, 0, 10);
	pAd->CommonCfg.RssiThr0 = Value;
	DBGPRINT(RT_DEBUG_TRACE, ("IF %s::(Rssi Thr0=%d)\n", __FUNCTION__, pAd->CommonCfg.RssiThr0));
	return TRUE;
}
	...
	IN	PRTMP_ADAPTER	pAd)
{
	if (IS_MT7601(pAd))
	{
		...
		SET_DEFAULT_THRESHOLD(pAd->CommonCfg.RssiThr0, -25);
