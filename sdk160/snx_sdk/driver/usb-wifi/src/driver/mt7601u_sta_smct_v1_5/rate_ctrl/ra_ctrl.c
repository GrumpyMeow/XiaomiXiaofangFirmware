	...
	IN RTMP_RA_LEGACY_TB *pTxRate)
{
	...
	// solve low probability NULL pointer issue 
	if (!pAd || !pEntry || !pTxRate || !pEntry->pTable || (pEntry->EntryType==0))	
	{	
		if(!pEntry)
		{
			DBGPRINT(1, ("pAd%sNULL, pEntry%sNULL, pTxRate%sNULL\n",
				pAd?"!=":"==", pEntry?"!=":"==", pTxRate?"!=":"=="));
		}
		else
		{
			DBGPRINT(1, ("pAd%sNULL, pEntry%sNULL, pTxRate%sNULL, pTable%sNULL, pEntry->EntryType=%d\n",
				pAd?"!=":"==", pEntry?"!=":"==", pTxRate?"!=":"==", pEntry->pTable?"!=":"==", pEntry->EntryType));
		}
		return; 
	}	
	// end
...
VOID MlmeNewTxRate(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry)
{
	...
	if (pEntry == NULL || pTable == NULL || (ULONG)pTable < 0x1000)
	{
	...
	if( pNextTxRate == NULL )    
	{        
		DBGPRINT_RAW(RT_DEBUG_ERROR,("pNextTxRate is set as invalid, return\n"));
		return;    
	}    
	// end
