	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (bGoBack && pAd->LatchRfRegs.Channel != pP2PCtrl->ListenChannel)
	{
//for ra0 ps sleep
			...
			&& ((pAd->LatchRfRegs.Channel == pAd->CommonCfg.Channel) ||(pAd->LatchRfRegs.Channel == pAd->CommonCfg.CentralChannel)))
		{
			...
			DBGPRINT(RT_DEBUG_TRACE, ("%s  -- Send PSM Data frame for off channel RM, SCAN_IN_PROGRESS=%d! pAd->LatchRfRegs.Channel=%d\n",
											__FUNCTION__, RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS), pAd->LatchRfRegs.Channel));
