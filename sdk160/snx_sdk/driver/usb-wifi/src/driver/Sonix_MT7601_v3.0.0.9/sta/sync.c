...
#endif /* WSC_STA_SUPPORT */
					{
						...
						*/
						...
						{	
							...
							if ((ie_list->bSelReg == 0) &&
								(CheckPbcInProbeRsp(pBssEntry->VarIeFromProbeRspLen, pBssEntry->pVarIeFromProbRsp) < 0))
		...
		if (is_my_bssid)
		{
...
#endif /* WSC_STA_SUPPORT */
			{
				if ((((pAd->StaCfg.WepStatus != Ndis802_11WEPDisabled) << 4) ^ ie_list->CapabilityInfo) & 0x0010)
				{
					...
					/* When using -Dwext and trigger WPS, do not check security. */
					if ((ie_list->bSelReg == 0) &&
						(CheckPbcInProbeRsp(pBss->VarIeFromProbeRspLen, pBss->pVarIeFromProbRsp) < 0))	
#endif /* NATIVE_WPA_SUPPLICANT_SUPPORT */
					{
						/*
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	*/
	...
	else
	{
		...
		if ( pAd->ToggleEnabled && (pAd->StaCfg.bWIDI && pAd->P2pCfg.bWIDI))
		{
			skip = TRUE;
			count_scan++;
