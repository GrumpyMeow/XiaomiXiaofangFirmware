...
#ifdef P2P_SUPPORT
	{"p2pWidiEnable", 				Set_P2PWiDiEnable_Proc},
#endif /* P2P_SUPPORT */
#endif /* WIDI_SUPPORT */
#ifdef AGS_SUPPORT
	{"Ags",						Show_AGS_Proc},
...
/*			kfree(pKey); */
	...
	case OID_HE_SET_PTK:
	{
		...
		{
			Status = copy_from_user(&pAd->WOW_Cfg.PTK, wrq->u.data.pointer, wrq->u.data.length);
			if (Status == 0)
			{
				DBGPRINT(RT_DEBUG_ERROR, ("Set::OID_HE_SET_PTK ok\n"));
			}
		}
		break;
	}
	...
	case OID_HE_SET_REPLAY_COUNTER:		
	{
		...
		{
			Status = copy_from_user(&pAd->WOW_Cfg.ReplayCounter, wrq->u.data.pointer, wrq->u.data.length);
			if (Status == 0)
			{
				DBGPRINT(RT_DEBUG_ERROR, ("Set::OID_HE_SET_REPLAY_COUNTER ok\n"));
			}
		}
		break;
	}
...
{
                INT                                                                         Len = 0;
                BOOLEAN                                                            bDpidFound, bSrFound;
                PUCHAR                                                               pData = NULL;
                PBEACON_EID_STRUCT                pEid;
                SHORT                                                               DevicePasswordID;
                PWSC_IE                                                              pWscIE;
		INT Ret=0;
                ...
                while (Len > 0)
                {
                                ...
                                while (Len > 0)
                                {
                                                ...
                                                if (be2cpu16(pWscIE->Type) == WSC_ID_SEL_REGISTRAR)
                                                {
                                                                ...
                                                                pWscIE->Data[0]=1;
                                                                DBGPRINT(RT_DEBUG_ERROR, ("YF_bSelReg = %d\n", pWscIE->Data[0]));
								bSrFound = TRUE;
                                                }
                                                ...
                                                if (be2cpu16(pWscIE->Type) == WSC_ID_DEVICE_PWD_ID)
                                                {
                                                                ...
                                                                DBGPRINT(RT_DEBUG_ERROR, ("YF_DevicePasswordID = 0x%04x\n", DevicePasswordID));
                                                                bDpidFound = TRUE;
                                                }
		...
		if (pVar[0] != 0xDD)
		{
			UCHAR INIT_OUI[] = {0xDD, 0x00};
			NdisCopyMemory(pVar, INIT_OUI, 2);
			VarIeLen += 2;

			NdisCopyMemory(pVar, WPS_OUI, 4);
			VarIeLen += 4;
			pVar[1] = 4;
			hex_dump("FAKE_WPS_IE_0", pVar, VarIeLen);
			Ret = VarIeLen;
		}

		if (!bSrFound && pVar[0] == 0xDD)
		{
			UCHAR SR_OUI[] = {0x10, 0x41, 0x00, 0x01, 0x01};
			NdisCopyMemory(pVar + VarIeLen, SR_OUI, 5);
			VarIeLen += 5;
			pVar[1] = VarIeLen - 2;

			DBGPRINT(RT_DEBUG_ERROR, ("FAKE YF_bSelReg %d\n", pVar[0]));
			hex_dump("FAKE_WPS_IE_1", pVar, VarIeLen);
			Ret = VarIeLen;
		
		}

		if (!bDpidFound && pVar[0] == 0xDD)
                {
                        UCHAR DPID_OUI[] = {0x10, 0x12, 0x00, 0x02, 0x00, 0x04};
                        NdisCopyMemory(pVar + VarIeLen, DPID_OUI, 6);
                        VarIeLen += 6;
                        pVar[1] = VarIeLen - 2;
                    
                        hex_dump("FAKE_WPS_IE_2", pVar, VarIeLen);
			Ret = VarIeLen;
                }
...
SHORT CheckPbcInProbeRsp(
                INT                                                                         VarIeLen,
                PUCHAR                                                               pVar)
{
                INT                                                                         Len = 0;
                BOOLEAN                                                            bFound, bSelReg;
                PUCHAR                                                               pData = NULL;
                PBEACON_EID_STRUCT                pEid;
                SHORT                                                               DevicePasswordID;
                PWSC_IE                                                              pWscIE;
	...
	IN	ULONG					Data)
{
	...
	for(IdBss=0; IdBss<pAd->ScanTab.BssNr; IdBss++)
	{
		...
		hex_dump("WPS_IE", pBssEntry->WpsIE.IE, pBssEntry->WpsIE.IELen);

		dpidBeacon = CheckPbcInProbeRsp(pBssEntry->WpsIE.IELen, pBssEntry->WpsIE.IE);
		dpidProbeRsp  = CheckPbcInProbeRsp(pBssEntry->VarIeFromProbeRspLen, pBssEntry->pVarIeFromProbRsp);

		if ((dpidBeacon < 0) && (dpidProbeRsp >=0))
		{			
			UCHAR realLen = 0;
			realLen = FakeWpsSR_Dpid(pBssEntry->WpsIE.IELen, pBssEntry->WpsIE.IE, dpidProbeRsp);
			pBssTable->WpsIeLen = realLen;
			
			hex_dump("WPS_IE_FINAL", pBssTable->pWpsIe, pBssTable->WpsIeLen);
		}
	...
	IN	ULONG					Data)
{
	...
	switch (pIoctlWpa->flags)
	{
    	...
    	case RT_CMD_STA_IOCTL_WPA_AUTH_80211_AUTH_ALG: 
			if (pIoctlWpa->value == RT_CMD_STA_IOCTL_WPA_AUTH_80211_AUTH_ALG_OPEN)        // this only available in 2.5.0.5, no found in other versions;
            	pAd->StaCfg.AuthMode = Ndis802_11AuthModeOpen;
...
INT set_str_suspend_enable(RTMP_ADAPTER *pAd)
{
	pAd->STR_Suspend_enable = TRUE;
	return 0;
}

INT clear_str_suspend_enable(RTMP_ADAPTER *pAd)
{
	pAd->STR_Suspend_enable = FALSE;
	return 0;
}

BOOLEAN test_str_suspend_enable(RTMP_ADAPTER *pAd)
{
	return pAd->STR_Suspend_enable == TRUE;
}
#ifdef ED_MONITOR
