...
unsigned int OnAuth(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	switch (pstat->state) {
	case WIFI_FW_AUTH_NULL:
		seq = cpu_to_le16(*(u16*)((SIZE_PTR)pframe + WLAN_HDR_A3_LEN + 2));
		algorithm = cpu_to_le16(*(u16*)((SIZE_PTR)pframe + WLAN_HDR_A3_LEN));

		/* algorithm == 0 open, algorithm == 1 shared */
		if (algorithm == 0 && seq == 1) {
			DBG_871X("Open system auth\n");
			pstat->state &= ~WIFI_FW_AUTH_NULL;
			pstat->state |= WIFI_FW_AUTH_SUCCESS;
			pstat->expire_to = pstapriv->assoc_to;
			pstat->authalg = algorithm;
			pstat->auth_seq = 2;
		} else if (algorithm == 1 && seq == 1) {
			/*shared key */
			DBG_871X("Shared key auth, create challenging text\n");
			pstat->chg_txt[0] = 0x35;
			pstat->chg_txt[127] = 0x53;
			pstat->state &= ~WIFI_FW_AUTH_NULL;
		...
		break;
	case WIFI_FW_AUTH_STATE:
		/* shared key */
		DBG_871X("Checking challenging txt...\n");
		pstat->state &= (~WIFI_FW_AUTH_STATE);
		pstat->state |= WIFI_FW_AUTH_SUCCESS;
...
//======================================================================================
	...
	//DBG_871X("auth alg=%x, seq=%X, auth_mode=%x\n", algorithm, seq, auth_mode);
	...
	DBG_871X("New4 auth alg=%x, seq=%X, auth_mode=%x\n", algorithm, seq, auth_mode);
	...
	else // shared system or auto authentication
	{
		if (seq == 1)
		{
			...
			printk("Create challenging text...\n");
		...
		else
		{
			...
			printk("Peter challenging txt check pass!!\n");
			pstat->auth_seq = 4;
